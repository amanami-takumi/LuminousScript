...
- `1-C-A-1` - チャプター1、ルートC配下のルートAのシーン1
- `1-C-M-1` - チャプター1、ルートC配下の収束ルートのシーン1
- `1-E` - チャプター1のエンディング
...
`script` 列は `,` 区切りで複数の directive を並べられます。

テキスト背景を透過：`<TEXT_BACK_OFF>`

警告ポップアップを表示する：`<NOTICE(メッセージ)>`

ジャンプの遷移先を変更する：`<JUMP_A(シーンID)>`
ABCDで変更して使う。
...
LocalStorage に文字列を書き込む：`<STATE_SET(route:A)>`

LocalStorage のキーを削除する：`<STATE_REMOVE(route)>`

LocalStorage の配列へ値を追加する：`<STATE_APPEND(visited:event_01)>`

比較が一致した時だけ directive を実行する：`<STATE_EQ(route:A)=TEXT_BACK_OFF>`

配列長が指定以上の時だけ directive を実行する：`<STATE_LEN_GTE(visited:10)=CLICK_DELAY(1000)>`

配列が値を含む時だけ directive を実行する：`<STATE_CONTAINS(visited:event_01)=AUTO_SCENE_CHANGE_DELAY(5000)>`

複数記述例：`<STATE_APPEND(visited:event_01),STATE_LEN_GTE(visited:10)=TEXT_BACK_OFF>`

`STATE_APPEND` は既存値が配列の時だけ追加を行います。キーが未作成なら配列で新規作成し、既存値が配列でない場合は何もしません。
